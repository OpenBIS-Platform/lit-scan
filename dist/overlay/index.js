...
export function drawOverlay(instance) {
    if (!overlayContainer)
        return;
...
function renderSingleOverlayBox(instance, rect, data) {
    if (!overlayContainer)
        return;
    ...
    // Update label
    const label = box.querySelector('.lit-scan-label');
    if (label) {
