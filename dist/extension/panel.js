...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LitScanPanelApp = class LitScanPanelApp2 extends i4 {
    constructor() {
      super(...arguments);
      this.updates = [];
      this.filterText = "";
    ...
    setupConnection() {
      ...
      this.port.onMessage.addListener((msg) => {
        if (msg.type === "COMPONENT_UPDATE") {
          this.updates = [msg.data, ...this.updates].slice(0, 50);
    ...
    render() {
      ...
      return b2`
            ...
            <input 
                ...
                @input=${(e5) => this.filterText = e5.target.value}
                ...
                ${visibleUpdates.map((u3) => b2`
                    <div class="log">
                        ...
                        <span style="color: ${u3.duration > 8 ? "#ffcc00" : "#4cd964"}">
                            ${u3.duration.toFixed(1)}ms
                        </span>
                    </div>
                `)}
                ${visibleUpdates.length === 0 ? b2`<em style="color: #666;">No component updates match your filter.</em>` : ""}
            </div>
        `;
    }
  };
